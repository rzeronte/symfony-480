...
reset-db: ## Drop and recreate database, make migration, and migrate
	@$(SYMFONY) doctrine:database:drop --force --if-exists
	@$(SYMFONY) doctrine:database:create --no-interaction
	@$(SYMFONY) doctrine:migrations:migrate --no-interaction

coverage: ## Run PHPUnit with coverage
	@$(DOCKER_COMP) exec -e XDEBUG_MODE=coverage php php ./bin/phpunit --coverage-html=./report

php-cs-fixer: ## Run PHP-CS-Fixer
	@$(DOCKER_COMP) exec -e PHP_MEMORY_LIMIT=-1 php php -d memory_limit=-1 ./vendor/bin/php-cs-fixer fix --diff --no-interaction
